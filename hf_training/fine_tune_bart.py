...
def main():
    ...
    def preprocess_function(examples):
        ...
        if 't5' in model_args.model_name_or_path or 'COGS' not in data_args.validation_file:
    ...
    if training_args.do_predict:
        ...
        split = "_".join(test_list[test_list.index('data') + 2].split('_')[:-1])
        if 't5' in model_args.model_name_or_path:
            model_name = 't5-base'
        else:
            model_name = 'bart-base'
            
        if 'gen' in data_args.validation_file:
            eval_split = 'gen'
        else:
            eval_split = 'test'
        logger.info("Writing model predictions to txt file...")
        with open(base_dir + '/preds/' + dataset_name + '/' + split + '/' + model_name + '_s' + str(training_args.seed) + '_' + eval_split + '.txt', 'w') as f:
